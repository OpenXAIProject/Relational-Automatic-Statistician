...
class GPModel:
    ...
    def __init__(self, mean=None, kernel=None, likelihood=None, nll=None, ndata=None, gpml_result = None, cov=''):
    ...
    def __hash__(self): return hash(self.__repr__())
    '''
    GPModel's magic method __repr__ now matches the sigature of __init__ method
    ...
    def kernel_expression(self):
        return self.kernel.get_gpml_expression(1)
    ...
    def my_from_matlab_output(output, model, ndata):
        ...
        model.kernel.load_param_vector(output.kernel_hypers)
        # model.likelihood.load_param_vector(output.lik_hypers)
        return GPModel(mean=MeanZero(), kernel=model.kernel, likelihood=LikGauss(sf=-np.Inf), nll=output.nll, ndata=ndata, gpml_result = output.gpml_result)
