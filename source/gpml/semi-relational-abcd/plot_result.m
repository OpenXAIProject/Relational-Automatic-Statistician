function [fS_mean f_mean] = plot_result(hyp, hyp_fS, hyp_f, cov, cov_fS, cov_f, sn, x, y, z, model)
...
fS_sigma = feval(cov_fS{:}, hyp_fS, x);
fS_sigma_star = feval(cov_fS{:}, hyp_fS, x, z);
fS_sigma_starstar = feval(cov_fS{:}, hyp_fS, z);
for i = 1:M
...
for i = 1:M
    ...
    plot_a_figure(x,y(:,i), z, complete_mean{i}, complete_var{i}, sub1, ['Complete components for time series ' num2str(i)]);
    sub2 = subplot(M, 3, (i-1)*3 + 2);
    plot_a_figure(x,y(:,i), z, fS_mean{i}, fS_var{i}, sub2, ['Shared component for time series ' num2str(i)]);
    sub3 = subplot(M, 3, (i-1)*3 + 3);
    plot_a_figure(x,y(:,i), z, f_mean{i}, f_var{i}, sub3,  ['Distinctive component for time series ' num2str(i)]);
end


end

function plot_a_figure(x, y, z, mu, sigma, sub, tit)
lw = 2;
fontsize = 10;
opacity = 1;
fake_opacity = 0.1;
